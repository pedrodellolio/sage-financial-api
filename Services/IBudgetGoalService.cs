...
namespace SageFinancialAPI.Services
{
    public interface IBudgetGoalService
    {
        ...
        Task<ICollection<BudgetGoal>> GetByBudgetMonthAndYearAsync(int month, int year, Guid budgetId);
        Task<ICollection<BudgetGoal>> GetAllAsync(Guid budgetId);
        Task<BudgetGoal> PostAsync(BudgetGoalDto request, Guid budgetId);
